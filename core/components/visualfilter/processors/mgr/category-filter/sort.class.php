...
class vfCategoryFilterSortProcessor extends modObjectProcessor {
    ...
    public function process() {
        ...
        $this->category = $source->get('category_id');
        ...
        if (!$this->modx->getCount($this->classKey, array('priority' => 0, 'category_id' => $this->category))) {
            $this->setIndex();
        }
    ...
    public function setIndex() {
        $q = $this->modx->newQuery($this->classKey, array('category_id' => $this->category));
        $q->select('id');
...
return 'vfCategoryFilterSortProcessor';